    ...
    <div class="carousel-wrap">
      {#each products as product, i (product.id)}
        <div class="carousel-item" class:active={i === current}>
          <a href={`/products/${product.id}`} class="carousel-img">
            <img src={product.image} alt={product.title} loading="lazy" />
          </a>
          <div class="carousel-overlay">
            <span class="carousel-cat">{product.category}</span>
            <a href={`/products/${product.id}`} class="carousel-title">{product.title}</a>
...
<style>
  ...
  .carousel-title { font-size:1.25rem; font-weight:700; color:var(--text); line-height:1.3; text-decoration:none; }
  ...
  .dot { width:10px; height:10px; padding:0; border-radius:50%; border:2px solid rgba(255,255,255,0.6); background:transparent; cursor:pointer; transition:0.15s; }
  ...
  .card { background:var(--bg-card); border-radius:var(--radius-lg); overflow:hidden; transition:transform 0.2s, box-shadow 0.2s; position:relative; }
  .card:hover { transform:translateY(-4px); box-shadow:var(--shadow-lg); }
  .card-img { display:flex; align-items:center; justify-content:center; padding:1rem; height:160px; background:var(--bg-alt); overflow:hidden; }
  ...
  .card-body { padding:0.65rem; display:flex; flex-direction:column; gap:0.25rem; }
  .card-cat { font-size:0.6rem; font-weight:600; color:var(--primary); text-transform:uppercase; letter-spacing:0.06em; }
  .card-title { font-size:0.78rem; font-weight:600; color:var(--text); line-height:1.3; display:-webkit-box; -webkit-line-clamp:2; -webkit-box-orient:vertical; overflow:hidden; text-decoration:none; }
  .card-title:hover { color:var(--primary); }
  .card-rating { display:flex; align-items:center; gap:0.2rem; font-size:0.7rem; color:var(--text-muted); font-weight:500; }
  .card-foot { display:flex; align-items:center; justify-content:space-between; margin-top:0.1rem; gap:0.5rem; }
